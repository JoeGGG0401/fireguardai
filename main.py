...
def intelligent_assistant():
    intelligent_assistant_file = open("消防演练智能助手.mp4", "rb")
    intelligent_assistant_bytes = intelligent_assistant_file.read()
    st.video(intelligent_assistant_bytes)

    st.markdown("""
    ### 智能助手

    我们的智能助手结合了强大的知识库和工具能力，专为帮助管理员进行消防演练而设计。通过智能助手，管理员可以轻松管理和执行复杂的消防演练任务，提高整体效率和准确性。

    #### 智能助手的核心特色：

    1. **知识库集成**
       - **全面的知识库**：智能助手内置了丰富的消防知识库，涵盖各种消防应急预案、操作规范和案例分析，帮助管理员快速获取所需信息。
       - **即时查询**：管理员可以随时通过智能助手查询消防知识，获取最准确和及时的指导。

    2. **工具能力**
       - **自动化操作**：智能助手能够自动执行消防演练中的关键操作，如启动报警系统、生成疏散路线图、触发灭火装置等，减少人为干预。
       - **智能识别**：结合机房监控系统，智能助手可以实时监控火灾信号，进行智能识别和处理，提高应急响应速度。

    3. **流程管理**
       - **预警审批**：智能助手支持预警审批流程，管理员可以通过助手进行预警确认或否决，确保每一步操作的准确性和有效性。
       - **通知发送**：智能助手自动生成并发送预警通知、安保通知和消防报警，确保信息的及时传达和处理。

    4. **数据支持**
       - **实时监控**：智能助手实时监控演练中的关键数据，并提供可视化展示，帮助管理员随时掌握演练进展。
       - **数据分析与报告生成**：演练结束后，智能助手自动分析演练数据，生成详细的演练报告，帮助管理员总结经验和优化流程。

    5. **高效协作**
       - **团队协作**：智能助手支持多用户协同工作，管理员可以通过助手共享信息和任务，提升团队协作效率。
       - **系统集成**：智能助手能够无缝集成企业现有的消防系统和工具，实现信息互通和功能扩展。

    总之，我们的智能助手通过先进的技术和贴心的设计，结合了丰富的知识库和强大的工具能力，帮助管理员高效地进行消防演练，确保每一次演练的成功和安全。
    """)
